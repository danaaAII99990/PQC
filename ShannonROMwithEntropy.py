...
class SharedState:
    ...
    def register_input(self, input_data, oracle_output):
        ...
        if hashed_input not in self.unique_inputs:
            ...
            # Update entropy using Shannon entropy instead of the previous method
            self.entropy = self.calculate_dynamic_entropy(oracle_output)

    def shannon_entropy(self, data):
        """Calculate Shannon entropy (bits) for a given byte sequence."""
        freq = Counter(data)  # Count occurrences of each byte
        length = len(data)
        return -sum((count / length) * math.log2(count / length) for count in freq.values())

    def calculate_dynamic_entropy(self, oracle_output):
        k_B = 1.380649e-23  # Boltzmann constant
        W = len(self.unique_inputs)  # Number of unique inputs (microstates)
        entropy_value = self.shannon_entropy(oracle_output)  # Compute Shannon entropy
        dynamic_entropy = k_B * math.log(W + 1) * entropy_value # boltzmann Entropy
...
class RandomOracle:
    def __init__(self, shared_state, output_size=64):
        self.oracle_data = {}
...
message2 = b"Hello, Quantum World!" * 128
rox_hash_output = rox_hash_with_dynamic_disorder_monitoring(message)

print(f"\nROX Hash Output: {rox_hash_output.hex()}")

#with open("hash_outputs.bin", "ab") as f:
   # for _ in range(1000000000000):  # Adjust this number to generate sufficient data
       # f.write(rox_hash_output)
