...
class RandomOracle:
    ...
    def query(self, input_data):
        ...
        # Print the entropy of the shared state
        print(f"Current Entropy in Oracle: {self.shared_state.get_microstate()}")
...
def simulate_thermal_equilibrium(data, temperature=1.0):
    """Simulate randomness based on the Boltzmann distribution."""
    k_B = 1.0  # Set Boltzmann constant to 1 for simplicity
    probabilities = []

    # Calculate probabilities based on Boltzmann distribution
    for byte in data:
        energy = byte / 255.0  # Normalize byte values to [0, 1]
        probability = math.exp(-energy / (k_B * temperature))
    ...
    # Generate new random output weighted by the probabilities
    output = random.choices(data, weights=probabilities, k=len(data))
    return bytes(output)

# Iterative mixing to mimic equilibrium
def iterative_mixing(data, iterations=10):
    """Perform iterative shuffling and XOR operations to simulate thermal mixing."""
    data = list(data)
    for _ in range(iterations):
        random.shuffle(data)  # Shuffle the data to simulate molecular motion
        mixed = [a ^ b for a, b in zip(data, data[::-1])]  # XOR with reverse
        data = mixed
    return bytes(data)
...
def rox_hash_with_dynamic_disorder_monitoring(message, block_size=64):
    ...
    with ThreadPoolExecutor() as executor:
        for block_index, block in enumerate(blocks):
            ...
            # Simulate thermal equilibrium
            disordered_hash = simulate_thermal_equilibrium(disordered_hash)

            # Iterative mixing
            disordered_hash = iterative_mixing(disordered_hash)
    ...
    #print(f"RO1 Entropy: {ro1.shared_state.get_microstate()}")
    #print(f"RO2 Entropy: {ro2.shared_state.get_microstate()}")
...
# Generate hash outputs and save to a file
#with open("hash_outputs.bin", "wb") as f:
    #for _ in range(10000):  # Generate 10,000 outputs
       # message = os.urandom(64)  # Random inputs
        #hash_output = rox_hash_with_dynamic_disorder_monitoring(message)
        #f.write(hash_output)
